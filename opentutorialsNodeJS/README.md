...
# 39 JavaScript - 객체 - 값으로서 함수
자바스크립트에서 함수는  
구문(statement)이면서,  
동시에 값(value)이기도 함.  
  
이런 특성을 이용하면 서로 연관된 데이터(변수)와 처리(함수)를 그루핑해서 정리정돈할 수 있음.  
이런 특성을 이용하면 소프트웨어의 복잡도를 낮출 수 있음.

## 값으로서
자바스크립트에서 배열과 객체는 모두 서로 연관된 데이터를 담는 그릇임.  
자바스크립트에서는 처리 방법을 그룹핑하는 함수조차도 데이터 이기도 때문에 배열과 객체에 함수를 담을 수 있다.

# 40 JavaScript - 객체 - 값으로서 함수
객체를 이용해서 프로그래밍하는 방법에 대해서 알아보자.

# 41 App - 객체를 이용해서 템플릿 기능 정리 정돈하기
객체를 이용해서 템플릿 기능을 정리 정돈하는 방법을 알아보자.

# 42 Node.js - 모듈의 형식
많아진 코드를 정리 정돈하는 가장 큰 도구인 모듈을 살펴 보도록 하자.